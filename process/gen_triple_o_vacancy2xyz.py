...
def save_configs(configs):
    """所有三O空位结构保存到 triple_o_vacancy.xyz"""
    atoms_list = []

    for removed, defect in configs:
        atoms = Atoms(
            symbols=[str(site.specie) for site in defect],
            scaled_positions=defect.frac_coords,
            cell=defect.lattice.matrix,
            pbc=True,
        )
        atoms.info["removed_O"] = " ".join(map(str, removed))
        atoms_list.append(atoms)

    write("mdstruct.xyz", atoms_list)
    print(f"\n✓ 共保存 {len(atoms_list)} 个结构到 mdstruct.xyz")
